...
\section{Position and alignment patterns}\label{sec:finder}
	...
	A |finder core| set by name still applies there: the ring keeps its tiles and only the center block becomes a shape, which is often all a code needs to stop looking generic.
	|finder core=inherit| hands the core back to the tiles, and leaves it empty when the ring is a shape and there are no tiles left to inherit from.
...
\begin{figure}[H]
	...
	\ShowFinder{finder core=star}{finder core=star}
...
\begin{table}
	\centering\begin{tabular}{>{\ttfamily}ll>{\ttfamily}cp{.5\linewidth}}
		...
		finder core     & shape   &  auto    & Shape of the center block of every pattern (|auto|, |inherit|, |none|, |square|, |rounded|, |circle|, or |star|); |auto| follows |finder|, any other value applies even when |finder| leaves the ring to the tiles (\autoref{sec:finder}). \\
